...
function pieChart() {
  ...
  var pullOutValuePostfix = "人";                     // Pull-out slice value postfix 
  ...
  function drawSlice ( context, slice ) {
    ...
    if ( slice == currentPullOutSlice ) {
      ...
      context.fillText( pullOutValuePrefix + chartData[slice]['value'] + pullOutValuePostfix + " (" + ( parseInt( chartData[slice]['value'] / totalValue * 100 + .5 ) ) +  "%)", centreX + Math.cos(midAngle) * ( chartRadius + maxPullOutDistance + pullOutLabelPadding ), centreY + Math.sin(midAngle) * ( chartRadius + maxPullOutDistance + pullOutLabelPadding ) + 20 );
