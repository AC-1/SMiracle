...
AryZone[15] = new Array("新興區","前金區","苓雅區","鹽埕區","鼓山區",
  "旗津區","前鎮區","三民區","楠梓區","小港區","左營區","仁武鄉","大社鄉","東沙群島",
  "南沙群島","岡山鎮","路竹鄉","阿蓮鄉","田寮鄉","燕巢鄉",
  "橋頭鄉","梓官鄉","彌陀鄉","永安鄉","湖內鄉","鳳山市","大寮鄉","林園鄉","鳥松鄉",
  "大樹鄉","旗山鎮","美濃鎮","六龜鄉","內門鄉","杉林鄉","甲仙鄉","桃源鄉","那瑪夏鄉",
  "茂林鄉","茄萣鄉");
...
function initAreaCode(areaId){
	...
	for (i = 0; i < AryAreaCode.length; i++) {
...
function changeZoneByClass(countyObj, zoneObj) {
	...
	for (i = 0; i < countyInput.length; i++) {
		...
		for (j = 0; j < AryZone[selectedCountyIndex].length; j++) {						
