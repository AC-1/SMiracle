...
public class ImageResizeUtil {
	  ...
	  private static final Log log = LogFactory.getLog(ImageResizeUtil.class);
	  
	  
	  
	  /**
	   * 
	   * @param inputFileName   輸入的檔案名稱
	   * @param Type            檔案型態 , 有 JPEG , GIF, PNG
	   * @param outWidth        輸出圖檔的寬
	   * @param outHeight       輸出圖檔的高
	   * @param outputFileName  輸出的檔案名稱
	   * @return boolean        轉換是否成功
	   */
	  @SuppressWarnings("finally")
	public boolean processImageResize(String inputFileName, String Type, int outWidth, int outHeight, String outputFileName){
	   boolean rtnValue = false;
	   FileOutputStream outputFile = null;
		  
	   try {
		    if(Type.equalsIgnoreCase("JPEG") || Type.equalsIgnoreCase("JPG")){
		       outputFile = new FileOutputStream(outputFileName); // 須為實體路徑及檔案名稱 
			   ImageIO.write(ImageResizeUtil.resizeImage(inputFileName, IMAGE_JPEG, outWidth, outHeight),"JPEG", outputFile);
			   outputFile.close();
			   rtnValue = true;
		    } else if(Type.equalsIgnoreCase("GIF")){
			   outputFile = new FileOutputStream(outputFileName); // 須為實體路徑及檔案名稱 
			   ImageIO.write(ImageResizeUtil.resizeImage(inputFileName, IMAGE_GIF, outWidth, outHeight),"GIF", outputFile);
			   outputFile.close();
			   rtnValue = true;
			} else if(Type.equalsIgnoreCase("PNG")){
		       outputFile = new FileOutputStream(outputFileName); // 須為實體路徑及檔案名稱 
			   ImageIO.write(ImageResizeUtil.resizeImage(inputFileName, IMAGE_PNG, outWidth, outHeight),"PNG", outputFile);
			   outputFile.close();
			   rtnValue = true;
		    }		   
	   } catch(Exception ex){
